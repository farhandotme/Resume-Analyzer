...
const findActionForFix = (fix: ResumeFix, actions: ActionPlanItem[], usedActions: Set<number>) => {
    let bestIndex = -1;
    let bestScore = 0;
    ...
    actions.forEach((action, index) => {
        ...
        const score = getRelevanceScore(fix, action);
    ...
    return clean(actions[bestIndex]?.action);
};
...
export default function Story6() {
    ...
    const improvements: Improvement[] = resumeFixes
        .filter((fix) => clean(fix.section) || clean(fix.fix) || clean(fix.why))
        .slice(0, 3)
        .map((fix, index) => {
